...
class KoreaInvestment:
    ...
    def get_execution_history(self, symbol: str, sort: str, start_date:str=str(datetime.datetime.today().strftime("%Y%m%d")), end_date:str=str(datetime.datetime.today().strftime("%Y%m%d"))):
        """해외주식체결내역 불러오기

        Args:
            symbol (str): symbol
            sort (str): "DS": 정순, "AS": 역순
            start_date (str, optional): 조회시작일자 (YYYYMMDD)
            end_date (str, optional): 조회종료일자 (YYYYMMDD)
        """
    ...
    def get_not_execution_list(self, sort: str=""):
        """해외주식 미체결 리스트 들고오기
    ...
    def get_period_profit(self, start_date:str, end_date:str):
        """해외주식 미체결 리스트 들고오기
